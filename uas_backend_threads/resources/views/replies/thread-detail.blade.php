...
<div style="border: 1px solid #ccc; padding: 15px; border-radius: 8px; max-width: 600px; margin-bottom: 20px; background: #fff;">
    <form action="{{ route('reply.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <textarea id="main-reply-text" name="content" placeholder="What's new?" rows="3" required style="width: 100%; border: none; outline: none; resize: none; font-size: 14px;"></textarea>
        ...
        <div style="display: flex; flex-direction: column; gap: 10px;">
            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
                <div style="display: flex; gap: 8px; align-items: center;">
                    ...
                    <button type="button" class="emoji-trigger-btn" data-target="main-reply-text" data-container="emoji-picker-main" style="cursor: pointer; border: 1px solid #ccc; padding: 4px 8px; border-radius: 4px; font-size: 13px; background: #f9f9f9;">
                        😊 Emoji
                    </button>
                </div>
                <button type="submit" style="background: #000; color: #fff; border: none; padding: 6px 15px; border-radius: 4px; font-weight: bold; cursor: pointer;">Post</button>
            </div>
...
<ul>
    @foreach($replies as $reply)
        <li style="margin-bottom: 25px; list-style: none; border-left: 2px solid #eee; padding-left: 10px;">
            <div>
                <strong>@if($reply->user){{ $reply->user->username }}@else user_anonim @endif</strong> 
            ...
            @if($reply->image)
                <div style="margin: 8px 0;">
                    <img src="{{ asset('storage/' . $reply->image) }}" alt="Media" style="max-width: 255px; max-height: 250px; border-radius: 6px; display: block;">
                </div>
            @endif
            ...
            <div style="margin-top: 5px;">
                ...
                @if($reply->content !== '[Komentar ini telah dihapus]' && $reply->user_id === auth()->id())
                    ...
                    <form action="{{ route('reply.destroy', $reply->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin ingin menghapus?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" style="font-size: 12px; color: red; background: none; border: none; padding: 0; cursor: pointer;">[Hapus]</button>
                    </form>
                @endif
            </div>
                        ...
                        <div style="display: flex; flex-direction: column; gap: 8px;">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <div style="display: flex; gap: 5px;">
                                    ...
                                    <button type="button" class="emoji-trigger-btn" data-target="reply-text-{{ $reply->id }}" data-container="emoji-picker-{{ $reply->id }}" style="cursor: pointer; border: 1px solid #ccc; padding: 2px 6px; border-radius: 4px; font-size: 11px; background: #fff;">
                                        😊 Emote
                                    </button>
                                </div>
